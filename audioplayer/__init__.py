...
application and the flashplayer from 1pixelout_.
...
Instead of using ``yes`` and ``no`` the ``audioplayer`` tag uses ``False``
and ``True``. 
...
Inserted code
=============
The following code is inserted by the ``audioplayer`` template tag (the
code is just an example and varies depending on your parameters)::
